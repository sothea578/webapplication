...
class booktour extends CI_Model
{
    ...
    {
        return $this->db->count_all('booktour');
    }

    public function show_user_bookTour($limit, $start)
    {
        $this->db->select("a.user_id, a.user_name, a.full_name, a.sex, a.email, a.phone_number, a.address, 
        b.id_bookTour, b.id_user, b.id_tour_type, b.id_cat, b.id_des, b.status, b.booking_date, 
        c.cat_id, c.cat_name, d.des_id, d.des_name, d.des_price, t.tour_type_id, t.tour_code")
    ...
    public function getReport(){
        $this->db->select("d.des_name, COUNT(b.id_user) as total")
            ->from('user as a, booktour as b, destination as d')
            ->where('a.user_id = b.id_user AND b.id_des = d.des_id');
        $query = $this->db->get();
        return $query->result();
    }
    ...
    {
        $this->db->select("a.user_id, a.user_name, a.full_name, a.sex, a.email, a.phone_number, a.address, 
        b.id_bookTour, b.id_user, b.id_tour_type, b.status, b.booking_date, 
        c.cat_id, c.cat_name, d.des_id, d.des_name, d.des_price, t.tour_type_id, t.tour_code")
            ->from('user as a, booktour as b, category as c, destination as d, tour_type as t')
    ...
    public function update_stt_book_tour($id, $id_tour)
    {
        $data = array(
        ...
        if ($id_tour != 1 && $this->input->post('book_stt') === 'confirm') {
            $this->db->set('people_amount', '`people_amount`+1', FALSE);
            $this->db->where('tour_type_id', $id_tour);
            $this->db->update('tour_type');
        }
    }
